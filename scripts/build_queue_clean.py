...
class CleanRow:
    ...
    sluttkunde: str
    naringstype: str
    moden_bestilling_dato: str | None
    kapasitet_ko_mw: float | None
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "moden_bestilling_dato": self.moden_bestilling_dato,
            "kapasitet_ko_mw": self.kapasitet_ko_mw,
...
def build_clean_row(raw: dict[str, str]) -> CleanRow:
    ...
    moden_bestilling_dato = parse_date_ddmmyyyy(raw.get("Dato - moden bestilling fra sluttkunde", ""))
    kapasitet_ko_mw = parse_mw(raw.get("Kapasitet i kø (MW)", ""))
    onsket_tilknytning = parse_date_ddmmyyyy(raw.get("Kundens ønskede dato for tilknytning", ""))
    ...
    return CleanRow(
        ...
        moden_bestilling_dato=moden_bestilling_dato,
        kapasitet_ko_mw=kapasitet_ko_mw,
...
def write_summary(rows: list[CleanRow]) -> None:
    total_mw = sum((r.kapasitet_ko_mw or 0.0) for r in rows)
    by_naringstype: dict[str, float] = {}
    by_prisomrade: dict[str, float] = {}

    for r in rows:
        mw = r.kapasitet_ko_mw or 0.0
...
def main() -> None:
    ...
    print(
        f"Wrote {OUT_CLEAN_CSV.name}, {OUT_CLEAN_JSON.name}, {OUT_SUMMARY_JSON.name} ({len(rows)} rows)"
    )
